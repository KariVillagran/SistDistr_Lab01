...
public class Cliente implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (conexion.iniciarRegistro(IPServer, Puerto)) {
    ...
    public void run() {
        ...
        IFinanzaRMI objetoRemotoHebraFinanza; //Se crea un nuevo objeto llamado objetoRemoto
        IRecursoHumanoRMI objetoRemotoHebraRRHH; //Se crea un nuevo objeto llamado objetoRemoto
        IUsuarioRMI objetoRemotoHebraLoginUser; //Se crea un nuevo objeto llamado objetoRemoto
        //Se instancia el objeto que conecta con el servidor
        ConexionCliente conexion = new ConexionCliente();
        try {
            //Se conecta con el servidor
            if (conexion.iniciarRegistro(IPServer, Puerto)) {
                ...
                objetoRemotoHebraFinanza=conexion.getServidorFinanza();
                objetoRemotoHebraLoginUser=conexion.getServidorLogin();
                objetoRemotoHebraRRHH=conexion.getServidorRRHH();
                while (true) {
